...
For.prototype = {
  ...
  compile: function(compiler) {
    //for循环
    compiler
      .write("var scope=context._pushScope({loop:{index:0},_iter_:true,_seq_:")
      .subcompile(this.iter)
      .raw("});");
    if (!this.key) {
      compiler
        .write("for(scope._iter_key_=0,scope.loop.length=scope._seq_.length;")
        .raw("scope._iter_key_<scope.loop.length;")
        .raw("scope._iter_key_++){")
        .indent()
        .write("scope." + this.value)
        .raw("=scope._seq_[scope._iter_key_];\n")
    } else {
      //for in 循环
      compiler
        .write("for(scope._iter_key_ in scope._seq_){")
        .indent()
        .write("scope.loop.index++;context.set(\"" + this.key + "\",scope._iter_key_);")
        .write("context.set(\"" + this.value + "\",scope._seq_[scope._iter_key_]);\n");
    }
    compiler
      .subcompile(this.body)
      .outdent()
      .write("}")
      .write("scope=context._popScope();\n");
  }
};
